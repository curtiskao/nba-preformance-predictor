...
def train_model(player_name):
    """Train a model for the specified player."""
    ...
    if len(df) < MIN_GAMES:
        ...
        print(f"   2. Choose a different player with more games")
        exit(1)
    ...
    print(f"\nNext step: python prediction.py --player \"{player_name}\" --opponent \"Team Name\" --home")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train NBA player points prediction model"
    )
    parser.add_argument(
        "--player",
        type=str,
